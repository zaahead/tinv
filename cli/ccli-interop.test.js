// cli/ccli-interop.test.js
// Proves the Rust ccli binary produces wire-compatible TINV3: encode a clip with
// the built `tinv` binary, then decode it with the WEB module and assert it is
...
const BIN = join(__dirname, "..", "ccli", "target", "release", "tinv");

const ffOk = hasSvtAv1();
const binOk = await access(BIN).then(() => true).catch(() => false);
const skip = !ffOk ? "ffmpeg/libsvtav1 unavailable" : !binOk ? "ccli binary not built" : false;
...
async function assertPlayable(tinvPath) {
  ...
  assert.ok(r && r.blob, "web module failed to decode ccli file");
...
test("ccli whole-file output decodes as valid fMP4 in the web module", { skip }, async () => {
  const dir = await mkdtemp(join(tmpdir(), "ccli_it_"));
  try {
    const src = await makeClip(dir, 2);
    const dst = join(dir, "clip.tinv");
    runCcli([src, "--preset", "talkinghead", "--min-split", "9999", "-o", dst]);
...
test("ccli force-chunked output decodes as valid fMP4 in the web module", { skip }, async () => {
  const dir = await mkdtemp(join(tmpdir(), "ccli_itc_"));
  try {
    const src = await makeClip(dir, 3);
    const dst = join(dir, "clip.tinv");
    // segment 1s + min-split 0 forces parallel segments on a 3s clip
    runCcli([src, "--preset", "talkinghead", "--segment", "1", "--min-split", "0", "-o", dst]);
    await assertPlayable(dst);
  } finally {
    await rm(dir, { recursive: true, force: true });
  }
